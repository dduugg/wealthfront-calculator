...
def dollar_allocation_for_risk(risk, allocations, dollars):
    dollar_allocation_for_risk = {}
    allocations_last_index = len(allocations) - 1
    allocation_index_to_interpolate = risk * allocations_last_index
    ...
    funds = set(lower_allocation) | set(upper_allocation)
    for fund in funds:
        interpolated_fund_weight = lower_weighting * lower_allocation.get(fund, 0) \
            + upper_weighting * upper_allocation.get(fund, 0)
        dollar_allocation_for_risk[fund] = dollars * interpolated_fund_weight / 100.0
    return dollar_allocation_for_risk
...
risk = min(risk, 1)
risk = max(risk, 0)
print "risk:", locale.format("%.1f", risk)
...
for fund in funds:
    ...
    amount_str = locale.format("%.2f", amount, grouping=True)
    max_amount_len = max(max_amount_len, len(amount_str))
    max_fund_len = max(max_fund_len, len(fund))
    combined_dollar_allocation[fund] = amount_str

for fund in combined_dollar_allocation:
    amount = combined_dollar_allocation[fund]
